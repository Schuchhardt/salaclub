export default ngModule => {
	ngModule.service('CypherService', () => {
		...
		const b64Decode = (data) => {
			...
			dataRef += "";
			do {
				h1 = b64Table.indexOf(dataRef.charAt(i++));
				h2 = b64Table.indexOf(dataRef.charAt(i++));
				h3 = b64Table.indexOf(dataRef.charAt(i++));
				h4 = b64Table.indexOf(dataRef.charAt(i++));
		...
		return {
			encode: (key, data) => {
				return b64Encode(xorEncrypt(key, data));
			},
			decode: (key, data) => {
				return xorDecrypt(key, b64Decode(data));
			}
		};
	});
};
